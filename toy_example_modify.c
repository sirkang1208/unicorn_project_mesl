#include <ctype.h>
#include <stdio.h>
#include <stdint.h>
#include <stdbool.h>
#include <stdlib.h>

const uint8_t SBox[256] = {
    
};

void ADD(uint32_t *out, uint32_t *in){
	out[1] = in[1] + in[2];
}

void MUL(uint32_t *out, uint32_t *in){
	out[1] = in[1] * in[2];
}

void LUT(uint32_t *out, uint32_t *in){
	out[1] = SBox[in[1]&0xFF];
}

void MIXs(uint32_t *out, uint32_t *in){
	int i;
	for (i = 0; i < 10; i++) {	
		out[3*i + 1] = in[3*i + 1] + in[3*i + 2];
		out[3*i + 2] = in[3*i + 1] * in[3*i + 2];
		out[3*i + 3] = SBox[in[3*i + 3]&0xFF];
		}
}

void RANDOM(uint32_t *data){
	data[0] = rand();
}

void RAND_XOR(uint32_t *out, uint32_t *in){
	uint32_t tmp[1];
	RANDOM(tmp);
	out[1] = in[1] ^ tmp[0];
}

// bool IO_RECEIVE(uint32_t* input){
//   for(int i = 0; i < 30;i++){
//     scanf("%d",*(input+i));
//     if(input[i] == "\n"){
//       break;
//     }
//   }
//   if(*(input) != NULL){
//     return true;
//   }
//   else{
//     return false;
//   }
// }

void IO_TRANSMIT(uint32_t* output){
  FILE* fp = fopen("result.txt","w");
  int output_len = sizeof(output)/sizeof(uint32_t);
  for(int i = 0; i < output_len; i++){
    fprintf(fp, "output[%d] = [%zu]", i , &output[i]);
  }
  fclose(fp);
}


void main(void){
  uint32_t Indata[1] = {0x01};
  uint32_t InData1[3] = {0x00, 0x42,0x58};
  uint32_t InData2[3] = {0x00, 0x37,0x94};
  uint32_t InData3[3] = {0x00, 0x40};
  uint32_t InData4[3] = {0x00,0x01,0x03,0x79,0x44,0x05,0x20,0x10,0x35,0x30,0x07,\
                              0x03,0x01,0x03,0x31,0x06,0x55,0x30,0x10,0x11,0x12,\
                              0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x20,0x21,0x22};
  uint32_t InData5[3] = {0x00, 0x52};
  uint32_t *OutData;
  bool RcvState = true; //IO_RECEIVE(InData);
  if(RcvState == true){
    if(InData[0] == 0x01){ 
      ADD(OutData, InData1);        
    }
    if(InData[0] == 0x02){
      MUL(OutData, InData2);        
    }
    if(InData[0] == 0x03){
      LUT(OutData, InData3);        
    }
    if(InData[0] == 0x04){
      MIXs(OutData, InData4);
    }
    if(InData[0] == 0x05){
      RAND_XOR(OutData, InData5);
    }
    IO_TRANSMIT(OutData);   
  }       
}
